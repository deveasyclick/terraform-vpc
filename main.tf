...
resource "aws_eip" "web_server_eip" {
  ...
  depends_on = [aws_internet_gateway.prod-gateway, aws_instance.web_server_instance]
}

output "aws_server_public_id"{
  value = aws_eip.web_server_eip.public_ip
}
# 9. Create Ubuntu server in the custom vpc
